...
Function MOTempCleanUp
  Delete $TEMP\myextensions.7z
  Delete $TEMP\myextensions.7z_0
  Delete $TEMP\myextensions.7z_1
...
Function MOConfigDownloadTimer.Callback
  ...
  ${If} $R0 == "error"
    MessageBox MB_OK|MB_ICONEXCLAMATION "$(MO_ERROR_EXT_DOWNLOAD)"
    GetDlgItem $0 $HWNDPARENT 1 ; Next button
    EnableWindow $0 1
...
Function MOExtDownloadTimer.Callback
  ...
  ${If} $R0 != "error"
    ${If} $MOCancelDownload == 0
      GetDlgItem $0 $HWNDPARENT 2 ; Cancel button
      EnableWindow $0 0
      ${NSD_SetText} $MOPromptLabel "$(MO_STATUS_INSTALL_EXT)"
      Mydownload::fnExecuteCmd '"$EXEDIR\7za.exe" x $TEMP\myextensions.7z -o$EXEDIR\core\distribution\myextensions\'
      Mydownload::fnMoveExtensionFiles "$TEMP\MozillaFirefox\core\distribution\myextensions\"
      ;WriteINIStr "$TEMP\MozillaFirefox\core\distribution\distribution.ini" "Preferences" "app.chinaedition.channel" ${Channel}
      WriteINIStr "$EXEDIR\core\distribution\myextensions\installdate.ini" "FILETIME" dwLowDateTime $R4
      WriteINIStr "$EXEDIR\core\distribution\myextensions\installdate.ini" "FILETIME" dwHighDateTime $R5
    ${Endif}
  ${Endif}
  GetDlgItem $0 $HWNDPARENT 1 ; Next button
  EnableWindow $0 1
...
!endif
  ...
  Mydownload::fnDownload /THREADNUMBER=2 ${MOExtUrl} myextensions.7z
FunctionEnd
